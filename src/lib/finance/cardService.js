...
/**
 * CardService — virtual card lifecycle via the active wallet & card provider.
 */
export const CardService = {
  async issue({ wallet_id, institution_id }) {
    const prov = CardProvider.get();
  ...
  async replace(id) { const c = await base44.entities.Card.get(id); const prov = CardProvider.get(); const res = await prov.issueCard(c.wallet_id); await base44.entities.Card.update(id, { status: "replaced" }); const nc = await base44.entities.Card.create({ wallet_id: c.wallet_id, masked_number: res.masked_number, type: "virtual", status: "active", daily_limit: c.daily_limit, monthly_limit: c.monthly_limit, institution_id: c.institution_id }); audit("Card replaced", id, `Replaced with ${res.masked_number}`); return nc; },
