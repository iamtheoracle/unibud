    ...
    <div className="min-h-screen">
      ...
      <div className="px-4 pb-2">
        <div className="relative">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search books, journals, papers..." className="w-full pl-9 pr-4 h-[44px] rounded-2xl bg-card border border-border/50 text-[13px] text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30" />
        </div>
      </div>

      <div className="px-4 pb-3">
        <div className="flex gap-2">
          <button onClick={() => setAiSearch(false)} className={`flex-1 h-[44px] rounded-2xl font-semibold text-[12px] flex items-center justify-center gap-1.5 transition-colors ${!aiSearch ? "bg-foreground text-background" : "bg-card border border-border/50 text-muted-foreground"}`}>
            <Search className="w-4 h-4" /> Search
          </button>
          <button onClick={() => setAiSearch(true)} className={`flex-1 h-[44px] rounded-2xl font-semibold text-[12px] flex items-center justify-center gap-1.5 transition-colors ${aiSearch ? "bg-primary text-primary-foreground shadow-[0_4px_20px_rgba(218,175,55,0.3)]" : "bg-card border border-border/50 text-muted-foreground"}`}>
      ...
      {aiSearch && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="px-4 pb-3">
          <div className="bg-card rounded-2xl p-3 premium-shadow border border-primary/20">
            <div className="flex gap-2">
              <input type="text" value={aiQuery} onChange={e => setAiQuery(e.target.value)} onKeyDown={e => e.key === "Enter" && handleAiSearch()} placeholder="Describe what you're looking for..." className="flex-1 px-3 h-[40px] rounded-xl bg-muted text-[13px] text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30" />
              <button onClick={handleAiSearch} disabled={aiLoading} className="px-3 h-[40px] rounded-xl bg-primary text-primary-foreground font-semibold text-[12px] disabled:opacity-50">{aiLoading ? "..." : "Find"}</button>
            </div>
      ...
      <div className="px-4 pb-3 flex gap-2 overflow-x-auto no-scrollbar">
        {CATEGORIES.map(cat => { const Icon = cat.icon; return (
          <button key={cat.key} className="flex-shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-full bg-card border border-border/50 text-[11px] font-semibold text-muted-foreground hover:text-foreground">
      ...
      <div className="px-4 space-y-6 pb-8 pt-1">
        ...
        {collections && collections.length > 0 && (
          <Section title="Collections & Course Folders">
            <div className="flex gap-3 overflow-x-auto no-scrollbar">
              {collections.map(c => (
                <motion.div key={c.id} whileTap={{ scale: 0.97 }} className="flex-shrink-0 w-[140px] bg-card rounded-[18px] p-3.5 premium-shadow border border-border/30 cursor-pointer">
                  <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center mb-2"><FolderOpen className="w-5 h-5 text-primary" /></div>
              ...
              <button className="flex-shrink-0 w-[140px] border-2 border-dashed border-border/40 rounded-[18px] flex flex-col items-center justify-center gap-1.5 text-muted-foreground hover:border-primary/30 hover:text-primary transition-colors">
        ...
        <Section title="Reading History">
          <div className="space-y-2">
            {recent.slice(0, 4).map(r => (
              <div key={r.id} onClick={() => openResource(r)} className="flex items-center gap-3 bg-card rounded-2xl p-3 premium-shadow border border-border/30 cursor-pointer">
                <div className="w-9 h-9 rounded-xl bg-primary/10 flex items-center justify-center"><History className="w-4 h-4 text-primary" /></div>
        ...
        <Section title="Citation Generator">
          <motion.div whileTap={{ scale: 0.98 }} className="bg-card rounded-2xl p-4 premium-shadow border border-border/30 flex items-center gap-3 cursor-pointer">
            <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center"><Quote className="w-5 h-5 text-primary" /></div>
