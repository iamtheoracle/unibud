...
import PresenceStatusSheet, { STATUS_META } from "@/components/shared/PresenceStatusSheet";
...
function Avatar({ name, status }) {
  ...
  return (
    <div className="relative shrink-0">
      <div className="w-11 h-11 rounded-full bg-primary/12 flex items-center justify-center text-primary font-semibold text-[16px]">
        {initial}
      </div>
      {dot && (
        <span className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-card ${dot}`} />
      )}
    </div>
  );
}
...
export default function Friends() {
  ...
  const { myPresence, byUser, setStatus, saving } = usePresence();
  ...
  return (
    <div className="w-full max-w-[600px] mx-auto px-5 pt-8 pb-32 safe-area-pt">
      <header className="mb-5">
        ...
        <button onClick={() => setStatusOpen(true)} className="mt-3 inline-flex items-center gap-2 px-3 py-1.5 rounded-full glass-card text-[12px] font-semibold text-foreground spring-tap">
          <span className={`w-2 h-2 rounded-full ${myPresence ? STATUS_META[myPresence.status]?.dot : "bg-muted-foreground"}`} />
          {myPresence ? STATUS_META[myPresence.status]?.label || "Set status" : "Set status"}
        </button>
      </header>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {tab === "friends" && (
            <FriendsList friends={friends} meId={meId} byUser={byUser} onUnfriend={(r) => remove(r.id)} loading={loading} />
          )}
          {tab === "requests" && (
      ...
      <PresenceStatusSheet open={statusOpen} onClose={() => setStatusOpen(false)} current={myPresence?.status} customMessage={myPresence?.custom_message} onSet={setStatus} saving={saving} />
    </div>
  );
}

function FriendsList({ friends, meId, byUser, onUnfriend, loading }) {
  if (friends.length === 0) {
    return (
  ...
  return (
    <div className="space-y-2.5">
      {friends.map((r) => {
        const isRecipient = r.recipient_id === meId;
        const name = isRecipient ? r.requester_name : r.recipient_name;
        const otherId = isRecipient ? r.created_by_id : r.recipient_id;
        const status = byUser?.[otherId]?.status;
        return (
