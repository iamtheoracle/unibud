...
import {
  ArrowLeft, Puzzle, Undo2, Recycle, Gauge, FlaskConical,
  GraduationCap, Expand, Lightbulb, Check, FileText,
} from "lucide-react";
import {
  EVOLUTION_PREAMBLE,
  EVOLUTION_CATEGORIES,
  EVOLUTION_RULES,
  EVOLUTION_GLOBAL_RULE,
  getEvolutionRulesByCategory,
} from "@/lib/constitution/evolutionConstitution";
...
export default function EvolutionConstitution() {
    ...
    <div className="w-full max-w-[520px] mx-auto px-5 pt-6 pb-32 safe-area-pt">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          <h1 className="text-[20px] font-bold tracking-tight text-foreground">Evolution Constitution</h1>
          <p className="text-[12px] text-muted-foreground">{EVOLUTION_RULES.length} permanent principles</p>
        </div>
      </div>
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-[13px] font-bold text-foreground">{EVOLUTION_PREAMBLE.title}</p>
        </div>
        <p className="text-[12px] text-muted-foreground leading-relaxed">{EVOLUTION_PREAMBLE.statement}</p>
      </motion.div>
      ...
      >
        ...
        <p className="text-[12px] text-foreground/90 leading-relaxed">{EVOLUTION_GLOBAL_RULE.rule}</p>
      </motion.div>

      {/* Categories */}
      {EVOLUTION_CATEGORIES.map((category, catIdx) => {
        const Icon = CATEGORY_ICONS[category.id] || Puzzle;
        const rules = getEvolutionRulesByCategory(category.id);
