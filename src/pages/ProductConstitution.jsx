...
import {
  ArrowLeft, Sparkles, Palette, Puzzle, Database, Brain, Wrench,
  Gauge, Rocket, Check, FileText,
} from "lucide-react";
import {
  PRODUCT_PREAMBLE,
  PRODUCT_CATEGORIES,
  PRODUCT_RULES,
  PRODUCT_GLOBAL_RULE,
  getProductRulesByCategory,
} from "@/lib/constitution/productConstitution";
...
export default function ProductConstitution() {
    ...
    <div className="w-full max-w-[520px] mx-auto px-5 pt-6 pb-32 safe-area-pt">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          <h1 className="text-[20px] font-bold tracking-tight text-foreground">Product Constitution</h1>
          <p className="text-[12px] text-muted-foreground">{PRODUCT_RULES.length} permanent principles</p>
        </div>
      </div>
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-[13px] font-bold text-foreground">{PRODUCT_PREAMBLE.title}</p>
        </div>
        <p className="text-[12px] text-muted-foreground leading-relaxed">{PRODUCT_PREAMBLE.statement}</p>
      </motion.div>
      ...
      >
        ...
        <p className="text-[12px] text-foreground/90 leading-relaxed">{PRODUCT_GLOBAL_RULE.rule}</p>
      </motion.div>

      {/* Categories */}
      {PRODUCT_CATEGORIES.map((category, catIdx) => {
        const Icon = CATEGORY_ICONS[category.id] || Sparkles;
        const rules = getProductRulesByCategory(category.id);
