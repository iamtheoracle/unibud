...
import {
  ArrowLeft, Command, TrendingUp, MousePointerClick, Layers, Target,
  Palette, Heart, Telescope, Check, FileText,
} from "lucide-react";
import {
  BUD_PRODUCT_VISION_PREAMBLE,
  BUD_PRODUCT_VISION_CATEGORIES,
  BUD_PRODUCT_VISION_RULES,
  BUD_PRODUCT_VISION_GLOBAL_RULE,
  getBudProductVisionRulesByCategory,
} from "@/lib/constitution/budProductVision";
    ...
    <div className="w-full max-w-[520px] mx-auto px-5 pt-6 pb-32 safe-area-pt">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          <h1 className="text-[20px] font-bold tracking-tight text-foreground">Bud Product Vision</h1>
          <p className="text-[12px] text-muted-foreground">{BUD_PRODUCT_VISION_RULES.length} permanent product principles</p>
        </div>
      </div>
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-[13px] font-bold text-foreground">{BUD_PRODUCT_VISION_PREAMBLE.title}</p>
        </div>
        <p className="text-[12px] text-muted-foreground leading-relaxed">{BUD_PRODUCT_VISION_PREAMBLE.statement}</p>
      </motion.div>
      ...
      >
        ...
        <p className="text-[12px] text-foreground/90 leading-relaxed">{BUD_PRODUCT_VISION_GLOBAL_RULE.rule}</p>
      </motion.div>

      {/* Categories */}
      {BUD_PRODUCT_VISION_CATEGORIES.map((category, catIdx) => {
        const Icon = CATEGORY_ICONS[category.id] || Command;
        const rules = getBudProductVisionRulesByCategory(category.id);
