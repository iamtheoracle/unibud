...
import { Search, Menu, X, Command, Bell, ChevronRight, Boxes } from "lucide-react";
import { Link } from "react-router-dom";
import CommandPalette from "@/components/oracle/CommandPalette";
import OracleContextSidebar from "@/components/oracle/OracleContextSidebar";
import { moduleById } from "@/lib/oracle/modules";

export default function OracleShell({ user, modules, active, onActive, children }) {
  ...
  const Brand = (
    <div className="px-4 py-4 border-b border-sidebar-border flex items-center gap-2.5">
      ...
      <div><p className="text-[14px] font-heading font-bold leading-none">Oracle</p><p className="text-[10px] text-muted-foreground mt-0.5">UNIBUD Operating Center</p></div>
    </div>
  );
        ...
        {mobileNav && (
          <>
            ...
            <motion.aside initial={{ x: -260 }} animate={{ x: 0 }} exit={{ x: -260 }} transition={{ type: "spring", stiffness: 380, damping: 36 }} className="fixed top-0 left-0 bottom-0 z-50 w-[244px] bg-sidebar flex flex-col lg:hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-sidebar-border"><span className="font-heading font-bold text-[14px]">Oracle</span><button onClick={() => setMobileNav(false)}><X className="w-5 h-5" /></button></div>
              <div className="flex-1 min-h-0">{NavList}</div>
            </motion.aside>
          </>
        )}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-14 shrink-0 flex items-center gap-3 px-4 border-b border-border bg-background/80 backdrop-blur-xl">
          ...
          <div className="flex items-center gap-2 text-[13px] text-muted-foreground hidden md:flex">
            <span>Oracle</span><ChevronRight className="w-3.5 h-3.5" /><span className="text-foreground font-medium">{activeModule?.label}</span>
          </div>
          <Link to="/architect" className="hidden md:inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-primary/15 text-primary text-[12px] font-heading font-semibold hover:bg-primary/25 spring-tap" title="Open Architect — platform builder">
            <Boxes className="w-3.5 h-3.5" />Architect
          </Link>
          <div className="flex-1 max-w-[420px] ml-auto">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <input value={search} onChange={(e) => setSearch(e.target.value)} onFocus={() => setPaletteOpen(true)} placeholder="Global search…" className="w-full h-9 pl-9 pr-16 rounded-lg bg-muted/40 border border-border text-[13px] focus:outline-none focus:border-primary/50" />
          ...
          <button className="p-2 rounded-lg hover:bg-muted/60 relative"><Bell className="w-5 h-5" /><span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 rounded-full bg-primary" /></button>
        ...
        <div className="flex-1 flex min-h-0">
          ...
          <aside className="hidden xl:flex w-[280px] shrink-0 border-l border-border bg-sidebar/40"><OracleContextSidebar module={activeModule} /></aside>
        </div>
      </div>
