...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-card/95 backdrop-blur-2xl border border-border/30 p-6 premium-shadow transition ease-in-out data-[state=closed]:duration-250 data-[state=open]:duration-400 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      side: {
  ...
  <SheetPortal>
    ...
    <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
      <SheetPrimitive.Close
        className="absolute right-5 top-5 flex items-center justify-center w-9 h-9 rounded-full glass opacity-50 ring-offset-background transition-all hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-secondary spring-tap">
