...
export default function OperatorShell({ user, institutionName, modules, active, onActive, children }) {
    ...
    <div className="h-screen flex bg-background overflow-hidden">
      <aside className="hidden lg:flex w-[240px] flex-col border-r border-border bg-sidebar/40 shrink-0">
        <div className="h-14 flex items-center gap-2 px-4 border-b border-border">
          ...
          <div className="min-w-0"><p className="font-heading font-bold text-[14px] truncate">Operator</p><p className="text-[10px] text-muted-foreground truncate">{institutionName || "Workspace"}</p></div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-14 flex items-center gap-2 px-4 border-b border-border shrink-0">
          ...
          <button onClick={() => setPalette(true)} className="flex items-center gap-2 h-9 px-3 rounded-xl glass text-muted-foreground text-[13px] flex-1 max-w-[420px]">
            <Search className="w-4 h-4" /><span className="flex-1 text-left">Search or jump…</span><kbd className="text-[10px] px-1.5 py-0.5 rounded bg-muted/60 font-mono">⌘K</kbd>
          </button>
          <div className="ml-auto flex items-center gap-2">
            <div className="text-right hidden sm:block"><p className="text-[12px] font-semibold leading-tight">{user?.full_name || "Operator"}</p><p className="text-[10px] text-muted-foreground capitalize">{user?.role || ""}</p></div>
            <div className="w-8 h-8 rounded-full bg-primary/20 grid place-items-center text-[12px] font-bold text-primary">{(user?.full_name || "O").slice(0, 1)}</div>
          </div>
        </header>
