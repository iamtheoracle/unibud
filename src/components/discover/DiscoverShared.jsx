...
export function ItemCard({ icon: Icon, title, subtitle, tag, right, to, color = "primary", image }) {
  ...
  const inner = (
    <div className="bg-card rounded-[20px] p-3.5 soft-shadow border border-border/40 flex items-center gap-3 card-hover">
      {image ? (
        <Image src={image} fittingType="fill" className="w-12 h-12 rounded-[14px] flex-shrink-0" />
      ) : (
        <div className={`w-10 h-10 rounded-[14px] ${c.bg} flex items-center justify-center flex-shrink-0`}>
          {Icon && <Icon className={`w-5 h-5 ${c.text}`} strokeWidth={2.1} />}
        </div>
      )}
      <div className="flex-1 min-w-0">
        <p className="text-[12px] font-semibold text-foreground truncate">{title}</p>
        {subtitle && <p className="text-[10px] text-muted-foreground truncate">{subtitle}</p>}
        {tag && <span className={`inline-block mt-1 px-2 py-0.5 rounded-full ${c.bg} ${c.text} text-[9px] font-semibold`}>{tag}</span>}
      </div>
...
export function EmptyHint({ icon: Icon, title, desc, action }) {
  return (
    <div className="flex flex-col items-center text-center py-10 px-6">
      <div className="w-14 h-14 rounded-[22px] bg-primary/8 soft-shadow flex items-center justify-center mb-3">
        {Icon && <Icon className="w-6 h-6 text-primary" strokeWidth={1.8} />}
      </div>
...
export function PromptCard({ icon: Icon, title, desc, color = "primary" }) {
  const c = COLOR[color] || COLOR.primary;
  return (
    <div className="rounded-[20px] p-4 bg-primary/5 border border-primary/15 mx-5">
      <div className="flex items-center gap-2 mb-1.5">
        {Icon && <Icon className={`w-4 h-4 ${c.text}`} />}
        <p className="font-heading font-semibold text-[13px] text-foreground">{title}</p>
      </div>
