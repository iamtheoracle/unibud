...
export default function LecturerShell({ user, instName, active, onActive, sections, onBack, children }) {
  ...
  const brand = (
    <div className="px-4 py-5 border-b border-sidebar-border">
      <p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Lecturer Portal</p>
      <h1 className="text-[16px] font-heading font-semibold leading-tight mt-1 truncate">{user?.full_name || "Lecturer"}</h1>
      <p className="text-[12px] text-muted-foreground mt-0.5 truncate">{instName || "UNIBUD"}</p>
    </div>
  );
          ...
          <>
            ...
            <motion.aside initial={{ x: -280 }} animate={{ x: 0 }} exit={{ x: -280 }} transition={{ type: "spring", stiffness: 380, damping: 36 }} className="fixed top-0 left-0 bottom-0 z-50 w-[260px] bg-sidebar flex flex-col md:hidden">
              <div className="flex items-center justify-between px-4 py-4 border-b border-sidebar-border">
                <span className="font-heading font-semibold text-[15px]">{user?.full_name || "Portal"}</span>
