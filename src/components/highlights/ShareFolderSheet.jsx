...
import { Share2, Copy, Check, Globe, Link2 } from "lucide-react";
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="rounded-t-[28px]">
        ...
        <div className="space-y-3 px-1 pb-8 pt-3">
          {!shared ? (
            <button
              onClick={handleShare}
            ...
            >
              <Globe className="w-4 h-4" /> Share with Community
            </button>
          ) : (
            <>
              <div className="flex items-center gap-2 p-3 rounded-[16px] bg-success/10 border border-success/20">
                <Check className="w-4 h-4 text-success shrink-0" />
                <p className="text-[12px] font-medium text-foreground">Collection is now public — other students can discover it in the Community tab.</p>
              </div>
