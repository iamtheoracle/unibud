...
export default function UniPortalSidebar({ user, collapsed = false, onNavigate }) {
  const location = useLocation();
  const navigate = useNavigate();
  const role = user?.role || "lecturer";
  const nav = getUniNav(role);
  ...
  return (
    <div className={`flex flex-col h-full glass-strong border-r border-border/30 transition-all duration-300 ${width}`}>
      ...
      <div className={`flex items-center gap-3 px-5 py-5 border-b border-border/20 ${collapsed ? "justify-center px-0" : ""}`}>
        ...
        {!collapsed && (
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[10px] text-muted-foreground font-medium mt-1">University Portal</p>
          </div>
        )}
        ...
        <div className="px-5 py-3 border-b border-border/20">
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[10px] text-muted-foreground truncate">{getUniRoleName(role)}</p>
            </div>
          </div>
        </div>
      ) : (
      ...
      <nav className="flex-1 overflow-y-auto py-3 no-scrollbar">
        {Object.entries(sections).map(([sectionName, items], si) => (
          <div key={sectionName} className="mb-3">
            {!collapsed && (
              <p className="px-5 text-[10px] font-semibold text-muted-foreground/70 uppercase tracking-wider mb-1.5">{sectionName}</p>
            )}
            {collapsed && si > 0 && <div className="mx-4 my-2 border-t border-border/20" />}
            {items.map((item) => {
              const Icon = ICON_MAP[item.icon] || LayoutDashboard;
              const isActive = item.path === "/uni-portal"
                ? location.pathname === "/uni-portal"
